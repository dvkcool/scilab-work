...
./configure --without-openmp CC=gcc-5 CXX=g++-5  CFLAGS=-fdiagnostics-color=always CXXFLAGS=-fdiagnostics-color=always --enable-static=no --without-tk --with-eigen_include=`pwd`/lib/Eigen/includes $parameters &&
